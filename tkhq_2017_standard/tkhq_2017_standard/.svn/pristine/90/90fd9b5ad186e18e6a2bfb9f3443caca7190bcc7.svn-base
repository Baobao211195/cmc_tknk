...
@SuppressWarnings("unchecked")
	...
	public List<TbdSysUsers> getAllUser() {
		Criteria cr = this.createCriteria();
		if(!Utils.getPrincipalLogin().getUser().getMaHq().equals("00"))
			cr.add(Restrictions.eq("maHq", Utils.getPrincipalLogin().getUser().getMaHq()));
	...
	public List<TbdSysUsers> searchByCondition(String username, String fullname, String donviHQ, Integer groupId,Integer active) {
		...
		if(!Utils.getPrincipalLogin().getUser().getMaHq().equals("00"))
			cri.add(Restrictions.eq("maHq", Utils.getPrincipalLogin().getUser().getMaHq()));
