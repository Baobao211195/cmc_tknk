angular.module('myApp').controller('TBSMahsDkghControllser', Tbs_CuaKhauNNController);
Tbs_CuaKhauNNController.$inject = ['$scope', 'TBSMahsDkghService', 'pagerService', '$uibModal', '$state', '$PopupMessage'];

function Tbs_CuaKhauNNController($scope, Tbs_CuaKhauNNService, pagerService, $uibModal, $state, $PopupMessage) {		
...
//
        function GetAllTBSMahsDkgh() {
        	Tbs_CuaKhauNNService.GetAllTBSMahsDkgh()
            	.then(
        			function (deferred) {
...
//        		);        	
    	...
    	function editRow(stt){
    		console.log('Id to be edited', stt);
            for (var i = 0; i < self.lstCuaKhauNN.length; i++) {
                if (self.lstCuaKhauNN[i].stt === stt) {
                    self.maHsdkghEntity = angular.copy(self.lstCuaKhauNN[i]);
                    break;
                }
            }
            self.maHsdkghEntity.addOrUpdate = 'update';
            self.showPopup(self.lstCuaKhauNN, self.maHsdkghEntity);
    	}    	
...
cuaKhaunnVnaccsModalCtrl.$inject = ['$uibModalInstance', 'grid', 'row', 'TBSMahsDkghService', '$PopupMessage']
function cuaKhaunnVnaccsModalCtrl($uibModalInstance, grid, row, Tbs_CuaKhauNNService, $PopupMessage) {
	...
	function saveData() {
		if (self.entity.addOrUpdate == 'add') {
        	...
        	updCuaKhauNN(self.entity, self.entity.stt);
        }
//		$uibModalInstance.close(self.message);
	...
	function updCuaKhauNN(entity, stt) {
		Tbs_CuaKhauNNService.updateTylePhiVanTai(entity, stt)
            .then(            
            	self.message = 'success',
                function (errResponse) {
