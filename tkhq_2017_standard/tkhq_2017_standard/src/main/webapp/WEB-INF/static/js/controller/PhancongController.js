...
myApp.controller('PhanCong_Controller', ['$scope',
	...
	$scope.pageChanged = function(currentPage) {
		...
		var begin = ((currentPage - 1) * $scope.numPerPage), end = begin
				+ $scope.numPerPage;
		$scope.phantrang = $scope.lstPhanCong.slice(begin, end);
	};
		...
		.then(
			function(deferred){
				...
				if (phanCongDto === null){
					...
					congViecEntiTy = getPhanCongEntity(idCv);
					if (congViecEntiTy.tuNgay === null){
						congViecEntiTy.tuNgay = '';
					}
					if (congViecEntiTy.denNgay === null){
						congViecEntiTy.denNgay = '';
					}
					showPopup ($scope.typeNhapXuat, congViecEntiTy, "1", $scope.congviec.maCv, $scope.congviec.tenCv );
				}
			},
		...
		Phancong_Service.GetPhanCongByMaCv(maCv, typeNhapXuat)
			.then(
				function(deferred){
					...
					$scope.phantrang = $scope.lstPhanCong.slice(begin, end);
					getTotalPage($scope.lstPhanCong.length);
				},
				function(errResponse){
					$PopupMessage.Error("Có lỗi trong quá trình xử lý thông tin.");
				}
