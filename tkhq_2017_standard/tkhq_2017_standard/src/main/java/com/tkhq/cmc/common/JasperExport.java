...
public class JasperExport {
	...
	public static ResponseEntity<byte[]> ExportReport(String reportName,
		...
		String reportFolderPath=fontPath.substring(0,fontPath.length()-9);
		
		parameter.put("PathReport", reportFolderPath);
		...
		try {
			...
			case EXCEL:
				...
				exporterXls.setExporterOutput(new SimpleOutputStreamExporterOutput(baos1));
				...
				xlsReportConfiguration.setIgnorePageMargins(Boolean.TRUE);
				xlsReportConfiguration.setOnePagePerSheet(Boolean.FALSE);
				xlsReportConfiguration.setCollapseRowSpan(Boolean.FALSE);
				xlsReportConfiguration.setColumnWidthRatio(new Float(1));
